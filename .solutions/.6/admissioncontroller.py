...
def validating_webhook():
    ...
    kind = request_info["request"]["object"]["kind"]
    objectname = request_info["request"]["object"]["metadata"]["name"]
    k8sobject = f'{kind}/{objectname}'    
    ...
    imagetag = request_info["request"]["object"]["spec"]["template"]["spec"]["containers"][0].get("image")
    if isinstance(imagetag, str):
        ...
        has_digest = "@" in imagetag
        result = True if has_digest else False
    else:
        result = False
